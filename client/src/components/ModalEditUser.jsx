...
export default function ModalEditUser({ open, onClose, onRefresh, user }) {
  ...
  useEffect(() => {
    ...
    if (user?.image) {
      setPreviewImage(`http://localhost:4000/uploads/${user.image}`);
    } else {
  ...
  const onSubmit = async (userData) => {
    const isSameData = Object.entries(userData).every(
      ([key, value]) => String(value) === String(user[key])
    );
        ...
        <DialogTitle sx={{ backgroundColor: "#374151", color: "#fff" }}>
          <div>
            <h2 className="text-lg font-semibold">
              <AddOutlinedIcon sx={{ fontSize: 24 }} /> Editar usuario
            </h2>
            <p className="text-sm text-gray-300 mt-1">
              Edita los datos y el avatar del usuario seleccionado
            </p>
          </div>
        </DialogTitle>
        <DialogContent
